...
// MQTT
String MQTT_TOPIC = "";
const char *MQTT_HOST = "mosquitto.cluster";
...
unsigned long previoussendToMqtt = 0;
...
void updateMqttConnection(void);
void displayShowText(String l1, String l2, String l3);
void updateTVOC(void);
void updateDispLedBar(void);
void updateCo2(void);
void updatePm(void);
void updateTempHum(void);
void sendToServer(void);
void connectToWifi(void);
String getNormalizedMac(void);
...
void setup()
{
  ...
  /** Show boot display */
  displayShowText("One V9", "Lib Ver: " + ag.getVersion(), "CUSTOM :)");
  ...
  MQTT_TOPIC = "airgradient/" + deviceID + "/";
...
void connectToMqttServer()
{
  displayShowText("MQTT Connecting", MQTT_HOST, "");
  if (!mqttclient.connect(deviceID.c_str(), MQTT_USERNAME, MQTT_PASSWORD, ("airgradient/" + deviceID + "/status").c_str(), 1, true, "offline"))
  {
    Serial.println("Failed to connect to MQTT server");
    displayShowText("MQTT Failed", MQTT_HOST, "");
    delay(MQTT_DELAY);
    return;
  }
  mqttclient.publish(("airgradient/" + deviceID + "/status").c_str(), "online", true);
  Serial.println("Connected to MQTT server, mills since start: " + String(millis()));
  displayShowText("MQTT Connected", MQTT_HOST, "");
  // mqttclient.subscribe(String(MQTT_TOPIC + "display/set").c_str());
  // mqttclient.subscribe(String(MQTT_TOPIC + "button/set").c_str());
  delay(1000);
  ...
  if (!bufferSizeIncreased)
  {
    ...
    if (HOASS_AUTO_DISCOVERY)
    {
      ...
      displayShowText("MQTT Connected", MQTT_HOST, "Buffer size increase failed");
      delay(5000);
...
void updateMqttConnection()
{
  ...
  {
    if (millis() - lastMqttConnectTry > mqttRetryInterval)
    {
...
void sendToMqtt()
{
  if (currentMillis - previoussendToMqtt >= SERVER_SYNC_INTERVAL)
  {
    Serial.println("Attempting to send to MQTT...");
    previoussendToMqtt = currentMillis;

    if (WiFi.status() == WL_CONNECTED && mqttclient.connected())
    {
      String measurements[] = {"pm25", "co2", "tvoc", "hum", "temp"};
      String displayNames[] = {"PM2.5 Sensor", "CO2 Sensor", "TVOC Sensor", "Humidity Sensor", "Temperature Sensor"};
      String units[] = {"µg/m³", "ppm", "ppb", "%", "°C", "", ""};
      String deviceClasses[] = {"pm25", "carbon_dioxide", "volatile_organic_compounds_parts", "humidity", "temperature"};
      String data[] = {String(pm25), String(Co2), String(TVOC), String(hum), String(temp, 2)}; // Include float temp and AQIDesc

      for (unsigned int i = 0; i < sizeof(measurements) / sizeof(measurements[0]); i++)
      {
        // skip sending negative values for all sensors except temperature and air quality
        if (data[i].toFloat() < 0 && displayNames[i] != "Temperature Sensor" && displayNames[i] != "Air Quality Description")
        {
          continue;
        }
        // but don't publish temperature if it's outside of normal range
        // because we can have a negative temperature on startup, so previous check will pass,
        // but it'll never be below -40 or above 150 because that's the sensor range
        if (displayNames[i] == "Temperature Sensor" && (data[i].toFloat() < -40 || data[i].toFloat() > 150))
        {
          continue;
        }

        publishSensorData(measurements[i], displayNames[i], units[i], deviceClasses[i], data[i]);
      }
    }
    else
    {
...
void publishSensorData(String measurement, String displayName, String unit, String deviceClass, String data)
{
  String publishTopic = String(MQTT_TOPIC) + measurement;
  // array of unsupported measurement types
  String unsupported[] = {"aqi_desc"};
  ...
  if (HOASS_AUTO_DISCOVERY)
  {
    for (unsigned int i = 0; i < (sizeof(unsupported) / sizeof(String)); i++)
    {
      if (measurement == unsupported[i])
        return; // Don't bother publishing data that HA can't understand
    }
    JSONVar doc;
    ...
    doc["availability_topic"] = String("airgradient/") + deviceID + "/status";
    doc["unique_id"] = deviceID + "_" + measurement;
    JSONVar device;
    JSONVar identifiers;
    identifiers[0] = deviceID.c_str();
    device["name"] = "AirGradient-" + deviceID;
    device["model"] = deviceID;
    device["manufacturer"] = "mqtt-airgradient";
